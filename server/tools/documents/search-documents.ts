...
export function registerSearchDocuments(server: McpServer, surface: SurfaceDefinition) {
  server.registerTool(
    "search_documents",
    {
      ...
      description: `Search the ${surface.label} surface when the exact topic is unknown. For large corpora, call list_sources first and pass a returned scope value, such as "sdks", "platforms/example-platform", or "example-platform". Use unscoped search only when broad cross-source search is explicitly needed. Results include URI, title, heading context, summary, and snippet.`,
